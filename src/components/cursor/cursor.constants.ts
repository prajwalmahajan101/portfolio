...
// Radar head — smaller than v1/v2. Sits directly on the cursor; the spring
// config is shared with the rest of the chain (see CHAIN_SPRING below) so the
// whole 9-link cascade feels homogeneous.
export const RADAR_R = 38;

// 5-ghost trail forming a chain with the head: the head sits on the cursor,
// ghost[0] springs off the head, ghost[i] springs off ghost[i-1].
...
export const GHOST_SCALES    = [0.88, 0.74, 0.60, 0.46, 0.32] as const;
export const GHOST_OPACITIES = [0.85, 0.66, 0.48, 0.30, 0.14] as const;

// One spring config used by every link in the chain (head + 5 ghosts). Damping
...
// (idleness never rises, isMoving stays true). Decay per RAF frame fixes it.
export const VELOCITY_DECAY = 0.86;

// Crescent offset — the WHOLE radar assembly (head + every ghost) is shifted
// by this distance behind the last-motion direction. Small while moving (cursor
// sits inside the radar), large at rest (cursor on the ring edge → moon shape).
// Applied identically to the head and each ghost so the cluster slides as one.
export const CRESCENT_INSIDE  = RADAR_R * 0.6;   // offset at full motion
export const CRESCENT_OUTSIDE = RADAR_R * 1.35;  // offset at full rest

// Idleness scalar (0 = full motion, 1 = full idle) drives the label/scan fade.
